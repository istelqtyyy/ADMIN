...
// Check if the logged-in user's level is ADMIN or FINANCE
if ($user_level !== 'ADMIN' && $user_level !== 'FINANCE') {
    die("You do not have permission to view this page.");
}

// SQL query to fetch login attempts for users with the level FINANCE only
$sql = "SELECT username, level, login, success FROM login WHERE level = 'FINANCE'"; // Changed 'attempted_at' to 'login'
...
echo "<title>Finance Login Attempts</title>";
echo "<style>
    body {
...
if ($result->num_rows > 0) {
    ...
    echo "<p>No login attempts found for Finance users.</p>";
}
