...
if (!isset($_SESSION['level'])) {
    die("You are not an ADMIN to view this monitoring!");
}

// Get user level from session
$user_level = $_SESSION['level'];

// Check if the logged-in user's level is ADMIN or HR
if ($user_level !== 'ADMIN' && $user_level !== 'HR') {
    die("You do not have permission to view this page.");
}

// SQL query to fetch login attempts for users with the level HR only
$sql = "SELECT username, level, login, success FROM login WHERE level = 'HR'";
...
echo "<title>HR Login Attempts</title>";
echo "<style>
    body {
...
if ($result->num_rows > 0) {
    ...
    echo "<p>No login attempts found for HR users.</p>";
}
